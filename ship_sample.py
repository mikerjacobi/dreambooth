...
# quality thresholds
vertical_symmetry_threshold = 0.53
nonzero_alpha_pixel_threshold = 50000

pipe = DiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", torch_dtype=torch.float16)
pipe.scheduler = DPMSolverMultistepScheduler.from_config(pipe.scheduler.config)
pipe.to("cuda")
pipe.load_lora_weights("mikerjacobi/blastro-ship-model")
...
while True:
  image = pipe("A picture of a sks spaceship in outer space", num_inference_steps=25).images[0]
  ...
  if vert_sym > vertical_symmetry_threshold and non_zero_alpha_pixels > nonzero_alpha_pixel_threshold: 
    keep += 1
    Image.fromarray(image_npy).save(f"projects/ships1/inferences/e_ship_{keep}.png")
